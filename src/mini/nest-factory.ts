...
class MiniApp {
  ...
  private getMeta(m: any): ModuleMeta {
    return Reflect.getMetadata('mini:module', m) ?? {};
  }
  private collectModules(root: any) {
    ...
    const dfs = (m: any) => {
      ...
      seen.add(m);
      out.push(m);
      const meta: ModuleMeta = this.getMeta(m);
  ...
  private registerExports(modules: any[]) {
    for (const m of modules) {
      const meta: ModuleMeta = this.getMeta(m);
      const exportsList: any[] = meta.exports ?? [];

      for (const token of exportsList) {
        if (typeof token === 'function') {
          this.container.register({ provide: token, useClass: token });
        }
      }
    }
  }
